...
- Allows you to create managed or self signed CM certificates and DNS zone for ALB on demand
- Create a security group with default rules and allows to define custom security rules
- Easy to use in other resources via outputs

## ALB definition

First of all, ALB load balancer requires next input data: 
1. one or several VPC networks with subnets (network id, zone names and subnet ids)
2. security group with all required ingress / egress rules
3. public DNS zone for ALB certificate, public zone id is required
4. a group of virtual machines, it will be a target group

Next step, define variable that describes all necessary components of ALB load balancer:
 - `alb_load_balancer` : a map of all ALB components (target groups, backend groups, http routers, virtual hosts, alb locations and alb listeners)

For ALB TLS listeners you need to:
1. set up `create_certificate` to `true`
2. define `alb_certificates` variable with all required certificates 
3. specify `public_dns_zone_id`
4. also you are able to set a timer interval with a `cert_waiting_timer`. This is a timer for waiting until certificate will be ISSUED by Let's Encrypt.
5. `certificate_ids` list is using for specifing uploaded certificates to Certificate Manager as a list of ids.
6. self managed certificates could be passed as strings or filenames. 

NOTE: `alb_load_balancer` variable describes parameters for all ALB component in a description section.

### ALB components dependencies

ALB load balancer components are linked together by a name. A top level component is ALB load balancer, next is a ALB listener, that points to ALB HTTP router and so on. Target group is a bottom level component.

A `canary_balancer` example shows these dependencies as a nested map of objects. `example-3-canary` example implements this scenario. You could find it in an examples folder.
...
This example create a ALB load balancer with two target groups, HTTP and GRPC backend groups, two HTTP routers, virtual hosts for each backend group and ALB listeners with ALB locations.
...
ALB certificates could be defined with `alb_certificates` variable. It supports two types of certificates - managed and self signed. Managed certificate will be issued by Let's Encrypt, it requires `domains` list, managed options such as `challenge_type` and `challenge_count`. Self signed certificates are created by your own using OpenSSL tool for example. 

ALB module is using following variables:
1. `create_certificate` - this flag enables or disables a block with a CM certificate.
2. `using_self_signed`  - set it to `true`, if you are using selt signed certificate.
3. `alb_certificates`   - a list of all certificates.
4. `public_dns_zone_id` - public DNS zone id for issuing certificates with Let's Encrypt.
5. `cert_waiting_timer` - a timer for waiting issued certificate from Let's Encrypt.

Next examples are describing managed and self signed certificates.
```
...
| <a name="input_dns_zone_labels"></a> [dns\_zone\_labels](#input\_dns\_zone\_labels) | Certificates default labels. | `map(string)` | <pre>{<br>  "component": "cloud-dns",<br>  "owner": "example"<br>}</pre> | no |
| <a name="input_enable_default_rules"></a> [enable\_default\_rules](#input\_enable\_default\_rules) | Controls creation of default security rules.<br><br>Default security rules:<br> - allow all incoming traffic from ANY protocol<br> - allows all outgoing traffic. Nodes can connect to Yandex Container Registry, Yandex Object Storage, Docker Hub, and so on<br> - allow access to Kubernetes API via port 6443 from subnet<br> - allow access to Kubernetes API via port 443 from subnet<br> - allow access to worker nodes via SSH from allowed IPs range | `bool` | `true` | no |
| <a name="input_folder_id"></a> [folder\_id](#input\_folder\_id) | The ID of the folder to which the resource belongs. If omitted, the provider folder is used. | `string` | `null` | no |
