...
resource "yandex_alb_backend_group" "backend_group" {
  ...
  dynamic "http_backend" {
    ...
    content {
      ...
      target_group_ids = concat([for target in http_backend.value.target_groups_list: yandex_alb_target_group.target_group[target].id])
      ...
      http2 = lookup(http_backend.value, "http2", null)
    }
  }
  ...
  dynamic "stream_backend" {
    ...
    content {
      ...
      target_group_ids = concat([for target in stream_backend.value.target_groups_list: yandex_alb_target_group.target_group[target].id])
  ...
  dynamic "grpc_backend" {
    ...
    content {
      ...
      target_group_ids = concat([for target in grpc_backend.value.target_groups_list: yandex_alb_target_group.target_group[target].id])
...
resource "yandex_alb_http_router" "http_router" {
  for_each    = length(var.alb_load_balancer.alb_http_routers) > 0 ? toset(var.alb_load_balancer.alb_http_routers) : toset([])
...
resource "yandex_alb_virtual_host" "virtual_router" {
  ...
  name           = each.key
  http_router_id = "${yandex_alb_http_router.http_router["${each.value.http_router_name}"].id}"
...
resource "yandex_cm_certificate" "alb_cm_certificate" {
  ...
  dynamic "self_managed" {
    ...
    content {
      certificate = try(self_managed.value.certificate_string, file("${path.module}/content/tls/${self_managed.value.certificate_filename}"))
      private_key = try(self_managed.value.private_key_string, file("${path.module}/content/tls/${self_managed.value.private_key_filename}"))
...
resource "yandex_alb_load_balancer" "alb_load_balancer" {
  ...
  allocation_policy {
    dynamic "location" {
      ...
      content {
        zone_id         = lookup(location.value, "zone_id", null)
